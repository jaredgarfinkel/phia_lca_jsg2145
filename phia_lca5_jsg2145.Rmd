...
lca_out_full = postClassProbs(awareARTVL.lca3) %>% 
  ...
  mutate(class = if_else(class_1 > class_2, 
         ...
         unaware = if_else(class == 1, 0, 1), # coded "backwards" meaning 0 is unaware, 1 is not unaware
         artaware = if_else(class == 2, 0, 1), # coded "backwards" meaning 0 is "aware not on art", 1 is not "aware not on art", or in other words, neither unaware nor aware and not on art - in terms of  class, this means that an individual may be EITHER "aware AND on art" OR unaware
         vlsaware = if_else(class == 3, 1, 0)) # coded "forwards" meaning 1 is aware on art, 0 is EITHER unaware OR "aware and not on art"
```
...
```{r}
xtabs(~ art + artaware, data = full_dat_out) %>% 
  ftable(.)
```

```{r}
xtabs(~vlunder200 + vlsaware, data = full_dat_out)
```
