...
Because the variables hivpsbp, hivrtpg and hivrslr are related to pregnancy, there are `r independent %>% filter(gender == 1) %>% nrow()` respondents who are not asked these questions.
...
There are `r ind_w %>% nrow()` in our women only data set.
...
There were `r ind_dat_out_w %>% filter(hivpsbp == 0, class == 1) %>% nrow()` false negatives. The sensitivity of this test is 346/1816, `r round(346/1816, 2)`, slightly higher, but still practically uninformative.
