...
```{r}
bio <- read_sas("C:\\Users\\jared\\OneDrive\\Mailman\\PHIA ICAP-GreenNewDeal2021\\camphia2017adultbio_20200109.sas7bdat") %>% 
  janitor::clean_names() %>% 
  mutate(across(.cols = c(-country, -householdid, -personid, -surveystdt, -resultvlc), ~as.numeric(.x)))


ind <- read_sas("C:\\Users\\jared\\OneDrive\\Mailman\\PHIA ICAP-GreenNewDeal2021\\camphia2017adultind_20200109.sas7bdat") %>% 
  janitor::clean_names() %>% 
...
bio_select <- bio %>% 
  ...
  # "avdprgmeth", "hivtstnors", "hivpostell", "sptgrgt", "stdl", "seekhelp", "unwantsxhelp")) %>%
    ...
    mutate( resultvlc = recode(resultvlc, 
            ...
            vlunder200 = if_else(resultvlc < 200, 1, 2, missing = 2)) 

bio_select %>% 
  group_by(vlunder200) %>% 
  summarize(n = n())
# 
# pull(bio, resultvlc)[c(812, 1312, 2169, 3522, 3568, 4716, 16262, 16318, 16911, 20511, 21634, 22443, 24188)]
```
...
simple_out4_conmat_df$class = factor(simple_out4_conmat_df$class, levels = c("reported negative" = 1, "aware on art" = 2, "aware not on art" = 3, "never tested" = 4), labels = c("not on art", "on art", "not on art", "not on art"))
...
simple_out4_conmat_df$class = factor(simple_out4_conmat_df$class, levels = c("reported negative" = 1, "aware on art" = 2, "aware not on art" = 3, "never tested" = 4), labels = c("unaware", "aware", "aware", "unaware"))
...
full_dat_out4_conmat_df$class = factor(full_dat_out4_conmat_df$class, levels = c("never tested" = 1, "aware not on art" = 2, "reported negative" = 3, "aware on art" = 4), labels = c("unaware", "aware", "unaware", "aware"))
...
full_dat_out4_conmat_df$class = factor(full_dat_out4_conmat_df$class, levels = c("never tested" = 1, "aware not on art" = 2, "reported negative" = 3, "aware on art" = 4), labels = c("not on art", "not on art", "not on art", "on art"))
...
full_dat_arv4_conmat_df$class = factor(full_dat_arv4_conmat_df$class, levels = c("never tested" = 1, "aware not on art" = 2, "aware on art" = 3, "reported negative" = 4), labels = c("unaware", "aware", "aware", "unaware"))
...
full_dat_arv4_conmat_df$class = factor(full_dat_arv4_conmat_df$class, levels = c("never tested" = 1, "aware not on art" = 2, "aware on art" = 3, "reported negative" = 4), labels = c("not on art", "not on art", "on art", "not on art"))
...
simpler_arv_conmat_df$class = factor(simpler_arv_conmat_df$class, levels = c("never tested" = 1, "aware on art" = 2, "reported negative" = 3, "aware not on art" = 4), labels = c("not on art", "on art", "not on art", "not on art"))
...
simpler_arv_conmat_df$class = factor(simpler_arv_conmat_df$class, levels = c("never tested" = 1, "aware on art" = 2, "reported negative" = 3, "aware not on art" = 4), labels = c("unaware", "aware", "unaware", "aware"))
...
NOTE: In the vlunder200 LCA model, the 5 variable model performs better than the 7 variable model in terms of kappa value. 

The arvstatus seem to perform similarly in the 5 and 7 variable models.

The 5 variable vlunder200 LCA model performs similarly to the arvstatus models.

In Cameroon, it appears a 5 variable model works better.

```{r}
modelvars_jsg <- c( "never_tested", "reported_negative", "reported_positive", "arvscurrent", "vlunder200")
# Remove all rows which have values other than 0 or 1
aware_patterns_jsg <- bio_select_mod %>% 
  select( all_of(modelvars_jsg) ) %>%
  filter( across(everything(), ~ .x %in% c(0,1)))
# aware_patterns$vlunder200
set.seed(719)
aware.lca_jsg <- randomLCA(aware_patterns_jsg, nclass = 5, quadpoints = 30)

outcomep_jsg <- round(aware.lca_jsg$outcomep, 4)
colnames(outcomep_jsg) <- names(aware_patterns_jsg)

OutcomeProbs.Table.jsg <- as.data.frame( cbind( Class = c(1, 2, 3, 4, 5), outcomep_jsg)) %>%
  mutate(across(-Class, as.numeric))
gt(OutcomeProbs.Table.jsg)
```
...
tab_cameroon = postClassProbs(awareARTVL.lca4) %>% 
  janitor::clean_names() %>% 
  mutate(class = if_else(class_1 > class_2, 
                         if_else(class_2 > class_3,
                                 if_else(class_3 > class_4, 1,
...
tab_cameroon %>% 
  gtsave("20201130-class_profiles_cameroon.png")
