...
```{r}
bio <- read_sas("C:\\Users\\jared\\OneDrive\\Mailman\\Summer 2020\\PHIA ICAP\\camphia2017adultbio_20200109.sas7bdat") %>% 
  janitor::clean_names() %>% 
  mutate(across(.cols = c(-country, -householdid, -personid, -surveystdt, -resultvlc), ~as.numeric(.x)))


ind <- read_sas("C:\\Users\\jared\\OneDrive\\Mailman\\Summer 2020\\PHIA ICAP\\camphia2017adultind_20200109.sas7bdat") %>% 
  janitor::clean_names() %>% 
